...
4. Choose `INPUT_METHOD = 'upload'` and upload one dataset ZIP when prompted, or choose `'drive'` and set `ZIP_PATH` to your ZIP in Google Drive. Any ZIP filename is supported. In the same runtime, the updated notebook reuses a single ZIP already present in its project folder; the dataset root and `valid`/`val` split are discovered automatically. If a full project ZIP also contains `data/processed/segmentation_dataset`, that generated copy is ignored in favor of the single original source root.
